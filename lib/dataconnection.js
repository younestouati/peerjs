...
function DataConnection(peer, provider, options) {
  ...
  this.options = util.extend({serialization: SERIALIZED_JSON}, options);
    ...
    case SERIALIZED_BINARY_UTF8:
      switch(data.constructor) {
        ...
        case String: // String fallback for binary data for browsers that don't support binary yet
          ...
          break;
        default:
          if (typeof Blob !== 'undefined' && data.contructor === Blob) { //Blob not defined in React Native, so test for undefined first
...
DataConnection.prototype._send = function(data, reliable, chunked) {
  ...
  switch(this.serialization) {
    ...
    case SERIALIZED_BINARY_UTF8:
      ...
      /*
         In order to avoid having to do any feature detection (which caused some problems in react native) we will just assume
         that binary blobs aren't support (chrome does not support it). Presumably this stuff will never be needed anyway. If proven
         wrong, look into reenabling it later
       */
...
DataConnection.prototype.handleMessage = function({type, name, data}) {
  switch (name) {
    case ANSWER:
      ...
      Negotiator.handleSDP(name, this, data.sdp);
      break;
    case CANDIDATE:
