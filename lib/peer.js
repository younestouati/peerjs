...
//const servers           = require('@phame/phame-shared/src/configuration/servers.json');
...
const servers = {nullValue: 'none'}; //TODO: FIX!!
...
function Peer(createSession, id, humanFriendlySessionId, options) {
  ...
  (createSession) ? this._createOrReclaimSession(id, humanFriendlySessionId) : this._initialize('join', id, humanFriendlySessionId);
}
...
Peer.prototype._createOrReclaimSession = function(sessionGuid, suggestedHumanFriendlySessionId) {
  const http = new XMLHttpRequest();
  let url = (this.options.secure ? 'https://' : 'http://') + this.options.host + '/createOrReclaimSession/' + (sessionGuid || servers.nullValue) + '/' + (suggestedHumanFriendlySessionId || servers.nullValue);
  url += '?ts=' + new Date().getTime() + '' + Math.random();
  ...
  http.onreadystatechange = () => {
    ...
    const {sessionGuid, humanFriendlySessionId} = JSON.parse(http.responseText).data;
    this._initialize('create', sessionGuid, humanFriendlySessionId);
  };
...
Peer.prototype._delayedAbort = function(type, message) {
  util.setZeroTimeout(() => this._abort(type, message));
};
